...
`src/clay_box_style_system/design_source/model.nim`. Service-specific MCP
...
Prototype module: `src/clay_box_style_system/testing/test_driver.nim`
...
code that imports `clay_box_style_system` should not receive test-driver APIs.
...
```text
src/clay_box_style_system/testing/integration/
...
- The top-level `clay_box_style_system` module should not export the test
  driver. Test code must opt in through `clay_box_style_system/testing/...`.
