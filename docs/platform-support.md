...
The portable suite is split deterministically across two Linux jobs, three
Windows jobs, and two macOS jobs. Windows receives the additional shard because
its per-test C compilation cost is substantially higher on hosted runners.
Every shard is release-blocking, and unit tests verify that each partition has
no overlap or missing entries. Cross-platform public-module, C ABI, header, and
Rust checks run once on shard zero for each platform. Native Cargo outputs are
cached by platform and lockfile, but cache misses still execute within the
configured timeout.
