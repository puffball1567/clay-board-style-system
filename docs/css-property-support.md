...
| `-cbss-custom-paint-mask` | Runtime | Resolves a registered bounded material as the alpha mask for the owner's isolated visual subtree. The deterministic PPM and SDL3 paths share destination-in semantics; this does not yet consume the general CSS `mask-*` metadata properties. |
