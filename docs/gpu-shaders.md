...
generated work bounded. Data-dependent loops remain outside the portable
authoring contract and must not be assumed by consumers.

### Typed pure helper functions

Compute packages can factor repeated formulas into generated, typed helper
functions without embedding backend source:
...
let scale = boundedRatio.callFunction([
  builder.scalar(2),
  builder.scalar(4)
])
```

Function and parameter names are generated by CBSS. Parameters, return values,
calls, lexical scope, and builder ownership are checked before source emission.
Helpers are compute-only and pure: they may use typed expressions, locals,
bounded loops, conditionals, and previously sealed helpers, but they cannot
declare or access uniforms, storage resources, images, stage built-ins, or main
outputs. A helper must finish with a top-level return. Nested definitions,
recursion, forward calls, and unbounded control flow are rejected. A builder is
limited to 64 helpers and 16 parameters per helper.
