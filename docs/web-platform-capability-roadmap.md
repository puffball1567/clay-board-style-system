...
| Effects, derived state, and subscriptions | First-party opt-in frontend-runtime module | Partial | Typed State, Store transactions, selectors, owned watch/effects, and bounded Commands are implemented. Cue orchestration remains planned. The module does not enter builds that omit it, and external state systems remain usable through ordinary Nim and Provider boundaries. |
